...
class TrainDataset(Dataset):
    def __init__(self, csv_path, dataset_path, input_size):
        self.data_info = pd.read_csv(csv_path, index_col=False,
                                     usecols=['Image path', 'Directory', 'Class ID', 'Class name'])  # Read the csv file
        self.data_info = self.data_info.sample(frac=1)
...
def get_train_dataloader(dataset_path, batch_size, input_size):
    train_set_path = "BarkNet_1_0/barknet_train.csv"
    train_data = TrainDataset(train_set_path, dataset_path, input_size)
